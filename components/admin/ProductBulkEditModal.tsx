...
interface ProductBulkEditModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (updates: { categoryId?: string; manufacturerId?: string; isVisible?: boolean; price?: number }) => void;
  categories: Category[];
...
export const ProductBulkEditModal: React.FC<ProductBulkEditModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setApplyPriceChange(false);
      setSelectedPrice('');
    }
  }, [isOpen, categories, manufacturers]);

  const handleSave = () => {
    const updates: { categoryId?: string; manufacturerId?: string; isVisible?: boolean; price?: number } = {};
          ...
          <div className="p-4 border rounded-md">
            <div className="flex items-center mb-2">
              <input
                ...
                className="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 mr-2"
              />
              <label htmlFor="applyPriceChange" className="font-medium text-gray-700">Change Price</label>
            </div>
            {applyPriceChange && (
              <Input
                label="New Price"
