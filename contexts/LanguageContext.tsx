...
export const LanguageProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const translate = useCallback((key: string, params?: Record<string, string | number>): string => {
    ...
    for (const k of keys) {
      console.log(`DEBUG: Current key part: ${k}, Current result:`, result); // Added debug log
      if (result && typeof result === 'object' && k in result) {
        result = result[k];
      } else {
        console.warn(`Translation key "${key}" not found for language "${language}". Path failed at: ${k}`); // Modified warning
        return key; // Return the key itself if not found
      }
    }
    
    if (typeof result === 'string') {
      console.log(`DEBUG: Resolved to string: ${result}`); // Added debug log
      if (params) {
    ...
    console.warn(`Translation key "${key}" did not resolve to a string for language "${language}". Resolved to:`, result); // Modified warning
