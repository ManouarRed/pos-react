...
import { MySalesAnalyticsPage } from '@/components/MySalesAnalyticsPage';
...
export const MySalesHistoryPage: React.FC<MySalesHistoryPageProps> = ({ currentUser }) => {
  ...
  const handleEditSale = (sale: SubmittedSale) => {
    setEditingSale(sale);
  ...
  const handleSaveEditedSale = async (updatedSale: SubmittedSale) => {
    try {
      await productService.updateSubmittedSale(updatedSale);
      await fetchSales(); // Re-fetch sales to update the list
      handleCloseEditModal();
    } catch (err) {
      console.error("Failed to update sale:", err);
      setError(t('errors.failedToUpdateData', { entity: t('common.sale') }));
    }
  };
