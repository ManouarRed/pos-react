...
export const SaleItemRow: React.FC<SaleItemRowProps> = ({ item, index, onUpdateItem, onRemoveItem }) => {
  ...
  const handleDiscountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const basePrice = item.isManual ? (item.manualPrice || 0) : (item.product?.price || 0);
    const quantity = item.isManual ? (item.manualQuantity || 0) : (item.quantity || 0);
  ...
  const handlePriceChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    onUpdateItem(index, { ...item, unitPrice: newPrice });
  };

  const handleSizeChange = (e: React.ChangeEvent<HTMLSelectElement | HTMLInputElement>) => {
    const newSize = e.target.value;
    if (isManual) {
      onUpdateItem(index, { ...item, manualSelectedSize: newSize, selectedSize: newSize });
      return;
    }
  ...
  const displayPrice = item.unitPrice !== undefined ? item.unitPrice : (isManual ? (item.manualPrice || 0) : (item.product?.price || 0));
  ...
  const showManualSizeInput = !isManual && (!item.product?.sizes || item.product.sizes.length === 0);
