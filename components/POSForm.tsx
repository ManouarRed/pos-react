...
export const POSForm: React.FC = () => {
  ...
  const addProductToSale = (product: Product) => {
    if ((product.totalStock ?? 0) === 0 && product.sizes.length > 0) {
    ...
    // If no sizes are defined for the product, allow manual input by setting selectedSize to empty string
    // and initialize unitPrice with product.price
    if (product.sizes.length === 0) {
        const newSaleItem: SaleItem = {
            id: uuidv4(),
            product,
            selectedSize: '', // Allow manual input
            quantity: 1,
            discount: 0,
            note: '',
            unitPrice: product.price, // Set initial unit price
        };
    ...
    // If product has sizes but the default selected size is out of stock
    ...
    if (existingItemIndex > -1) {
        ...
        // Add if stock is available
        if (defaultSizeStock > 0) {
            const newSaleItem: SaleItem = {
  ...
  const saleItemsWithFinalPrice: SaleItemWithFinalPrice[] = useMemo(() => {
    return saleItems.map(item => {
      const basePrice = item.isManual ? item.manualPrice! : (item.unitPrice !== undefined ? item.unitPrice : item.product!.price);
  ...
  const totalAmount = useMemo(() => {
    const subtotal = saleItemsWithFinalPrice.reduce((sum, item) => sum + item.finalPrice, 0);
    return subtotal - discount;
  }, [saleItemsWithFinalPrice, discount]);

  const handleSubmitSale = async (discount: number) => {
  ...
  return (
    <div className="bg-white p-6 sm:p-8 rounded-xl shadow-xl space-y-6"> 
      {submissionStatus && submissionStatus.type === 'error' && (
        <div className="bg-red-100 text-red-700 p-4 rounded-md text-sm mb-4" role="alert">
      ...
      {submissionStatus && submissionStatus.type === 'success' && (
        <SuccessAnimation
          ...
          onAnimationComplete={() => setSubmissionStatus(null)}
        />
      )}
      ...
      <div className="mt-6 pt-6 border-t border-gray-200">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="text-right w-full sm:w-auto space-y-2">
            <div className="flex items-center justify-end space-x-2" style={{ display: 'block !important', visibility: 'visible !important' }}>
              <p className="text-lg font-medium text-gray-600">{t('posForm.discountLabel')}</p>
              <Input
                type="number"
                value={discount}
            ...
            <div className="flex flex-col sm:flex-row sm:justify-end gap-3">
              ...
              <Button
                onClick={() => handleSubmitSale(discount)}
